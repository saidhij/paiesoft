...
class HrEmployee(models.Model):
    ...
    jrs_trav_cemois = fields.Integer(string="Jours travaillés (mode horaire)",compute='jours_travail')
    hrs_trav_cemois = fields.Integer(string="Heures travaillées (mode horaire)",default=0)
    jrs_trav_cemois_m = fields.Integer(string="Jours travaillés (mode mensuel)",default=26)
    hrs_trav_cemois_m = fields.Integer(string="Heures travaillées (mode mensuel)",default=191)	
    ...
    mode_pay = fields.Selection(string="Type de Paie(voir contrat)",  related='contract_id.pay_period')
    # mode_pay = fields.Selection([('monthly', 'Mensuel'),('hourly', 'Horaire')], string='Période de Paie', index=True, default='monthly')   

    @api.depends('hrs_trav_cemois')
    def jours_travail(self): 
        for s in self:
            s.jrs_trav_cemois = (s.hrs_trav_cemois*26)/191 

...
class HrContract(models.Model):
    ...
    wagecalc= fields.Monetary('Salaire horaire', help="Taux horaire",compute='_get_wage_cal')	
    wageh = fields.Monetary('Salaire horaire', help="Taux horaire",related='wagecalc')
    ...
    moistrav = fields.Float(string="Nombre de mois à considérer pour le congé",default=0.0)		
    ...
    @api.depends('moistrav')
    def _get_leave(self):
        for r in self:		
            r.leave=r.moistrav*1.5 
    ...
    @api.depends('wage','mode_pay')
    def _get_wage_cal(self):
        for w in self: 
            if w.mode_pay == 'hourly':
                w.wagecalc = 14.81
            if w.mode_pay == 'monthly':				
                w.wagecalc = w.wage/191 
